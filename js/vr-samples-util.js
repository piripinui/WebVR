...
window.VRSamplesUtil = (function () {
  ...
  // PJR added id for styling.
  function addButtonElement (message, key, icon, id) {
    ...
    if (icon) {
		if (id) {
			buttonElement.innerHTML = "<img src='" + icon + "' id='" + id + "'/><br/>" + message;
		}
		else {
  ...
  function addButton (message, key, icon, id, callback) {
    ...
    var element = addButtonElement(message, key, icon, id);
